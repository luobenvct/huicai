	...
	methods:{
		...
		openUserBtn(){
			...
			if (this.messageVerification_put== "") {
				...
				this.itemnum++;
				var holdername =  decodeURI(HC.vueurlFun("holder"));
				var bankName =  decodeURI(HC.vueurlFun("bankName"));
				axios.post('/api/v2/account/depository/deal_password/reset',{
					// 支付密码
				...
				.then(res=>{
					...
					if(res.data.code == "SUCCESS") {
						this.$toast("修改成功");
						var returnUrl = HC.getStorage("returnUrl") || '';
						if(returnUrl==''){
							setTimeout(()=>{
		...
		/*获取短信验证码*/
		gainFun(){
			axios.post('/api/v2/account/depository/get/account_mobile',{
				app_token:this.token,
			})
			.then(res=>{
				if (res.data.code == "SUCCESS") {
