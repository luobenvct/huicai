<template>
	<div class="coupons">
		...
		<div class="app_box">
			<img class="max_img" v-if="!rechargeShow" src="../../assets/deposits_img/open004.png">
		<img class="max_img" v-else src="../../assets/deposits_img/buyin10.png">
		<div class="max_title">存管账户充值</div>
		<div class="order_num"> 
		...
		<div class="succ_box">
			...
			<div class="succ_two succ_width" id="succ_show">
				...
				<div class="succ_time">{{items.expectedTimes}}</div>
			</div>
...
export default {
	...
	methods:{
		...
		rechargeFun : function(){
			axios.post('/api/v2/account/depository/recharge/details',{
			...
			.then(res=>{
				if(res.data.code=="SUCCESS"){
					...
					if(res.data.result.status=="U"){
						...
						res.data.result.amountName = res.data.result.amount+"元充值中";
					}else if(res.data.result.status=="S"){
						this.rechargeShow = true;
						res.data.result.amountName = "成功充值"+res.data.result.amount+"元";
					}
